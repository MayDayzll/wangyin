package cn.com.jmw.m.netease.session;
...
import cn.com.jmw.m.netease.session.extension.StickerAttachment;
import cn.com.jmw.m.netease.session.viewholder.MsgViewHolderDefCustom;
import cn.com.jmw.m.netease.session.viewholder.MsgViewHolderSticker;
import cn.com.jmw.m.netease.session.viewholder.MsgViewHolderTip;
...
public class SessionHelper {
    ...
    private static SessionCustomization normalTeamCustomization;//讨论组
...
//        NimUIKit.startChatting(context, tid, SessionTypeEnum.Team, getTeamCustomization(tid), backToClass, anchor);
    ...
    private static SessionCustomization getP2pCustomization() {
        if (p2pCustomization == null) {
            ...
            //在MessageFragment中手动取消了默认添加图片，视频、位置信息菜单项，在添加聊天界面“+”加号点开更多面板中包含的操作时、
            // 根据需求在此处添加相应的Action。orz
...
//                AckMsgInfoActivity.start(context, message);
        ...
        NimUIKit.setMsgForwardFilter(new MsgForwardFilter() {
            ...
            public boolean shouldIgnore(IMMessage message) {
                if (message.getMsgType() == MsgTypeEnum.custom && message.getAttachment() != null &&
                        (message.getAttachment() instanceof SnapChatAttachment)) {//删除了白板消息和红包消息附件，需要时自行添加。orz
    ...
    private static void registerMsgRevokeFilter() {
        NimUIKit.setMsgRevokeFilter(new MsgRevokeFilter() {
            ...
            @Override
            public boolean shouldIgnore(IMMessage message) {
                if (message.getAttachment() != null && (message.getAttachment() instanceof AVChatAttachment)) {//删除了白板消息和红包消息附件，需要时自行添加。orz
